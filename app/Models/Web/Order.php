...
class Order extends Model
{
    ...
    public function place_order($request)
    {
        $donation = 0;
        // $total_price_ship=0
        $total_price_donate = 0;
        $total_price_donatesss = "1";
        $bigorderdata = $request->all();
        $subtotal_donate = 0;
        $cart = new Cart();
        $result = array();
        ...
        $delivery_suburb =  session('shipping_address')->building;
        $delivery_city = session('shipping_address')->city;
        $delivery_postcode = session('shipping_address')->postcode;
        $delivery_phone = session('shipping_address')->billing_phone;
        ...
        $delivery_country = isset($country[0]->countries_name) ? $country[0]->countries_name : 'Dubai';
        $countries_iso_code_2  = isset($country[0]->countries_iso_code_2) ? $country[0]->countries_iso_code_2 : 'UAE';
        $currency  = isset($country[0]->currency) ? $country[0]->currency : 'AED';
        $phonecode  = isset($country[0]->phonecode) ? $country[0]->phonecode : '971';
        ...
        $billing_suburb = session('billing_address')->billing_building;
        ...
        if (isset(session('billing_address')->DialCode)) {
            $DialCode = session('billing_address')->DialCode;
        } else {
            $DialCode = "";
        }
        ...
        $billing_country = isset($country[0]->countries_name) ? $country[0]->countries_name : '0';
        ...
        $tax_rate = round(number_format((float) session('tax_rate'), 2, '.', ''));
        $coupon_discount = number_format((float) session('coupon_discount'), 2, '.', '');
        $order_price = (session('products_price') + $tax_rate + $shipping_price) - $coupon_discount + $request->method_Rates;
        ...
        //dd($shipping_method);
        $orders_status = '1';
        //$orders_date_finishguest_checkout
        ...
        if ($payment_status == 'success') {
            $total_lpointsToUse = 0;

            // print_r($request->all());die;

            if ($request->lpointChecker == 'added') {
                //orderprice * convertionRate
                // $total_lpointsToUse = auth()->guard('customer')->user()->redeem_points / $request->lpoints_conversion;
                // $lpointNorderprice = $order_price - $total_lpointsToUse;

                // if ($lpointNorderprice == 0) {

                //     DB::table('users')->where('id', auth()->guard('customer')->user()->id)->update(['redeem_points' => 0]);
                // } elseif ($lpointNorderprice < 0) {

                //     DB::table('users')->where('id', auth()->guard('customer')->user()->id)->update(['redeem_points' => abs($lpointNorderprice)]);
                // } else {

                //     DB::table('users')->where('id', auth()->guard('customer')->user()->id)->update(['redeem_points' => 0]);
                // }

                // $order_price = $order_price - $total_lpointsToUse;
                $total_lpointsToUse = auth()->guard('customer')->user()->redeem_points - $request->remaining_point;
                $order_price = $request->product_price;
                DB::table('users')->where('id', auth()->guard('customer')->user()->id)->update(['redeem_points' => $request->remaining_point]);
                
            }

            if (isset($request->method_Rates)) {
                $shipping_cost = $request->method_Rates;
            } else {
                $shipping_cost = 0;
            }
            ...
            $orders_id = DB::table('orders')->insertGetId(
                [
                    ...
                    'total_tax' => $total_tax,
                    'ordered_source' => '1',
                    'delivery_phone' => $DialCode . ' ' . $delivery_phone,
                    'billing_phone' => $DialCode . ' ' . $billing_phone,
                    'lpoints_points' => $request->order_lpoints,
                    'spent_lpoints' => $total_lpointsToUse,
                    'transaction_id' => $request->ref,
                ]
            );

            if (auth()->guard('customer')->user()->refered_by_status == 1) {
                $lpoints_id = DB::table('lpoints')->insertGetId(
                    [
                        'orders_id' => $orders_id,
                        'customers_id' => $customers_id,
                        'lpoints_amount' => $request->referral_conversion,
                        'lpoints_points' => $request->referral_conversion,
                        'type' => 2,
                    ]
                );
                $lpoints_id = DB::table('lpoints')->insertGetId(
                    [
                        'orders_id' => $orders_id,
                        'customers_id' => auth()->guard('customer')->user()->refered_by,
                        'lpoints_amount' => $request->referral_conversion,
                        'lpoints_points' => $request->referral_conversion,
                        'type' => 2,
                    ]
                );
                DB::table('users')->where('id', auth()->guard('customer')->user()->id)->update(['refered_by_status' => 0]);
                DB::table('users')->where('id', auth()->guard('customer')->user()->id)->increment('redeem_points', $request->referral_conversion);
                DB::table('users')->where('id', auth()->guard('customer')->user()->refered_by)->increment('redeem_points', $request->referral_conversion);
            }
            $lpoints_id = DB::table('lpoints')->insertGetId(
                [
                    'orders_id' => $orders_id,
                    'customers_id' => $customers_id,
                    'lpoints_amount' => $request->lpoints_conversion,
                    'lpoints_points' => $request->order_lpoints,
                    'type' => 1,
                ]
            );
            DB::table('users')->where('id', $customers_id)->increment('redeem_points', $request->order_lpoints);
            ...
            foreach ($cart_items as $products) {
                ...
                for ($i = 0; $i < $products->customers_basket_quantity; $i++) {


                    $ticket_id = DB::table('tickets')->insertGetId(
                        [
                            'orders_id' => $orders_id,
                            'campaign_id' => $products->campaign_id[0]->id,
                            'products_id' => $products->products_id,
                            'customers_id' => $customers_id,
                            'reward_id' => $products->campaign_id[0]->reward_id,
                            'created_at' => date('Y-m-d H:i:s'),
                            'type' => "O",
                        ]
                    );
                    $ticketNumber = DB::table('tickets')->where("products_id", $products->products_id)->where("reward_id", $products->campaign_id[0]->reward_id)->where('campaign_id', $products->campaign_id[0]->id)->where('type', 'O')->count();
                    $ticketNumber = $ticketNumber++;
                    DB::table('tickets')->where('id', $ticket_id)->update(['ticket_number' => $ticketNumber]);

                    if ($request->totalCouponChecker == 'added') {
                        $donation = 1;
                        // DB::table('orders')->where('id',$orders_id)->update([
                        //     'shipping_cost' => 0
                        // ]);
                    ...
                    if (isset($bigorderdata["earlyBirdChecker_" . $products->products_id])) {
                        if ($bigorderdata["earlyBirdChecker_" . $products->products_id] == 'added') {
                            $early =  DB::table('tickets')->where('campaign_id', $products->campaign_id[0]->id)->where('type', 'E')->count();
                            $total_early =  DB::table('campaigns')->where('id', $products->campaign_id[0]->id)->value('no_customers');
                            if ($early < $total_early) {
                                DB::table('tickets')->insertGetId(
                                    [
                                        'orders_id' => $orders_id,
                                        'campaign_id' => $products->campaign_id[0]->id,
            ...
            foreach ($order as $data) {
                ...
                $total_price_donate = $total_price;
                $subtotal_donate = $total_price;
                $data->data = $product;
                $orders_data[] = $data;
            }
            ...
            $ordersData['totalCouponChecker'] = $request->totalCouponChecker;
            ...
            // $delivery_phone = session('shipping_address')->billing_phone;
            if ($donation == 0) {
                // dd($total_price_donate);
                $shippAdd = array(
                    "firstName" => session('shipping_address')->firstname,
                    "lastName" => session('shipping_address')->lastname,
                    "address1" => session('shipping_address')->street,
                    "address2" => "",
                    "phone" => session('shipping_address')->billing_phone,
                    "city" => session('shipping_address')->city,
                    "state" => $countries_iso_code_2,
                    "postcode" => $phonecode,
                    "country" => array(
                        "name" => $delivery_country,
                        "code" => $countries_iso_code_2
                    ),
                    "company" => "Winztime"
                );
                // dd($cart_items);
                $postData = array(
                    "api_key" => "a3c65c2974270fd093ee8a9bf8ae7d0b",
                    "number" => (string)$orders_id,
                    "status" => 'New',
                    ...
                    "totals" => array(
                        ...
                        "subTotal" => $total_price_donate,
                        "discount" => 0,
                        "tax" => 0,
                        "shipping" => $shipping_cost,
                        "total" => $total_price_donate
                    ),
                    "note" => "This is an api test order",
                    "shippingMethod" => "Aramex",
                    "shippingAddress" => $shippAdd,
                    "billingAddress" => $shippAdd,
                    "items" => $items,
                    "meta" => array(
                        "villa_number" => 10,
                        "street_number" => "RS12D"
                ...
                curl_setopt($ch, CURLOPT_URL, "https://www.shorages.com/api/v1/order/create/");
            ...
            $returndata = array('payment_status' => $payment_status, 'orders_id' => $orders_id);
            return $returndata;
    ...
    {
        $campaignInfo = DB::table('tickets')->where('orders_id', '=', $order_id)->orderBy('ticket_number', 'Desc')->get();
        return $campaignInfo;
    }
    ...
    public function getPages($request)
    {
        ...
        $pages = DB::table('pages')
            ...
            ->where([['pages.status', '1'], ['type', 2], ['pages_description.language_id', session('language_id')], ['pages.slug', $name]])
            ->get();
        //   dd(DB::getQueryLog());
        return $pages;
    }
