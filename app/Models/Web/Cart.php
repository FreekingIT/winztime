    ...
    public function myCart($baskit_id)
    {
        ...
        if ($total_carts > 0) {
            foreach ($baskit as $cart_data) {
                $cart_data->total_order = DB::table('tickets')->where('campaign_id', $cart_data->campaign_id)->where('type', 'O')->count();

                $pro_ids[] = $cart_data->products_id;
                //products_image
                ...
                $reward_id = Campaign::where('id', $cart_data->campaign_id)->pluck('reward_id');

                $cart_data->campaign_id = Campaign::where('id', $cart_data->campaign_id)->get();
                if (!empty($reward_id)) {
                    $cart_data->reward = Reward::where('id', $reward_id)->get();
                } else {
                    $cart_data->reward = array();
                }
                // $cart_data->total_order = DB::table('orders_products')->where('campaign_id',$cart_data->campaign_id[0]->id)->groupBy('orders_id')->count();


                // $cart_data->campaign_id = Campaign::where('product_id',$cart_data->products_id)->pluck('id');
...
// dd($cart_data);
                ...
                // $result['reward'] = Campaign::where('product_id',$cart_data->products_id)->where('reward_id',$reward_id)->get();
            }
        }
        // dd($result);
        return ($result);
