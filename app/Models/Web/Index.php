...
use App\Models\Web\HomeWinners;
use DB;
...
class Index extends Model
{
   ...
   $winners = HomeWinners::leftJoin('image_categories', 'home_winners.winner_image', '=', 'image_categories.image_id')
            ->select('home_winners.id',
                'home_winners.winner_title',
                  'winner_image','winner_prize',
                'image_categories.path',
                   'image_categories.alt_tag'
            )
            ->where('status', '=', '1')
            ->where('image_categories.image_type', '=', 'actual')
            ->where('languages_id', '=', session('language_id'))
            ->orderBy('home_winners.id', 'desc')
            ->get();

        if (empty($winners) or count($winners) == 0) {

            $slides =  HomeWinners::leftJoin('image_categories', 'home_winners.winner_image', '=', 'image_categories.image_id')
            ->select('home_winners.id',
                'home_winners.winner_title',
                'winner_image','winner_prize',
                'image_categories.path',
            ...
            if ($menus) {
                ...
                foreach ($menus as $parents) {
                    ...
                    if ($parents->type == 0) {
                        ...
                        $link = ' href="' . url('pagedddd?name=') . $parents->link . '"';
                    } elseif ($parents->type == 3) {
                        $link = ' href="' . url('shop?category=') . $parents->link . '"';
                    } elseif ($parents->type == 4) {
                        $link = ' href="' . url('product-detail/') . $parents->link . '"';
