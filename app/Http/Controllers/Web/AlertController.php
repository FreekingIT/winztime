...
{
	public function checkapiresponse($customer_id, $six_digit_random_number)
	{
		try {
			$customers = DB::table('users')->where('id', $customer_id)->first();
			Mail::send('/mail/otpverification', ['userData' => $customers, "six_digit_random_number" => $six_digit_random_number], function ($m) use (
				$customers,
				$six_digit_random_number
			) {
				$m->to($customers->email)->subject("One time Password for verification")->getSwiftMessage()
	...
	public function createAlertPasswordChanged($existUser)
	{

		try {

			Mail::send('/mail/ChangePasword', ['userData' => $existUser], function ($m) use ($existUser) {

				$m->to($existUser[0]->email)->subject("Password Changed Successfully")->getSwiftMessage()
	...
	public function createAlertPasswordChangedreset($existUser)
	{

		try {

			Mail::send('/mail/ResetPasword', ['userData' => $existUser], function ($m) use ($existUser) {

				$m->to($existUser->email)->subject("Password Changed Successfully")->getSwiftMessage()
		...
		try {
			if ($alertSetting[0]->create_customer_email == 1 and !empty($existUser[0]->email)) {
				...
				Mail::send('/mail/createAccount', ['userData' => $existUser], function ($m) use ($existUser) {
					...
					$m->to($existUser[0]->email)->subject("Welcome To Winztime")->getSwiftMessage()
		...
		if ($alertSetting[0]->order_email == 1) {
			...
			if (!empty($ordersData['orders_data'][0]->admin_email)) {
				...
				try {
					...
					Mail::send('/mail/orderEmail', ['ordersData' => $ordersData], function ($m) use ($ordersData) {
						$m->from('support@winztime.com', 'Winztime');
						$m->to($ordersData['orders_data'][0]->admin_email)->subject("Winztime: An order has been placed")->getSwiftMessage()
			...
			if (!empty($ordersData['orders_data'][0]->email)) {
				...
				try {
					...
					Mail::send('/mail/orderEmail', ['ordersData' => $ordersData], function ($m) use ($ordersData) {
						$m->from('support@winztime.com', 'Winztime');
						$m->to($ordersData['orders_data'][0]->email)->subject("Winztime: An order has been placed")->getSwiftMessage()
	...
	public function forgotPasswordAlert($existUser)
	{
		...
		if ($alertSetting[0]->forgot_email == 1 and !empty($existUser[0]->email)) {
			...
			Mail::send('/mail/recoverPassword', ['existUser' => $existUser], function ($m) use ($existUser) {
				...
				$m->to($existUser[0]->email)->subject("Winztime Password Recovery")->getSwiftMessage()
