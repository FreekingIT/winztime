...
class CustomersController extends Controller
{
    ...
    public function display()
    {
        ...
        $customers = $this->Customers->paginatorAll();
    ...
    public function insert(Request $request)
    {
        ...
        if (count($existEmail)> 0 ) {
            ...
            if ($request->addresstype == 'Home') {
               $request->addresstype_home = 1;
            } else {
               $request->addresstype_office = 1;
            }
            $address_book_id = DB::table('addresses')->insertGetId([
            	'user_id'=>$customers_id,
                'first_name'	 	=>	 $request->entry_firstname,
                'last_name'   		=>   $request->entry_lastname,
                'address'	=>   $request->entry_street_address,
                'buildingno' 			=>   $request->buildingno,
                'countries_id'	 	=>	  $request->countries_id,
                'city'   			=>   $request->city,
                'default_address'		=>   1,
                'delivery_phone'		=>   $request->delivery_phone,
                'addresstype_home'		=>   $request->addresstype_home,
                'addresstype_office'		=>   $request->addresstype_office,
            ]);
           
            return redirect('admin/customers/edit/' . $customers_id)->with('update', 'Customer has been created successfully!');
        }
    }
    ...
    public function diplayaddress(Request $request){
        ...
        $customer_addresses = $this->Customers->addresses_new($id);
    ...
    public function deleteAddress(Request $request){
      ...
      $customer_addresses = $this->Customers->deleteNewAddresses($request);
